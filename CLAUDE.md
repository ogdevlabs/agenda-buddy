...
- **Result/validation (F-019):** FluentResults (`Result`/`Result<T>` returned by every Booking command/query handler, replacing a string-sniffed `"exception"`-prefix convention), Validot (declarative `Specification<T>` DSL — partially migrated, see the Requirement 6 note under `Booking.Api/`), GuardClauses (package id `GuardClauses`, not Ardalis — `GuardClause.ArgumentIsNotNull`). Mapster is ADR-049-approved for this line but has no call sites yet
...
- **Testing:** xUnit — **991 tests total**, in **three separate suites** that no single command runs: **516** across 12 backend test projects (`agenda-buddy-backend.slnf`; the slnf also carries `Gateway` itself, a 13th, non-test project), **310** in `AgendaBuddy.IntegrationTests` (real services — now including the Gateway — over HTTP against a MongoDB Testcontainer — needs a container runtime; F-018 Waves 1a–2a added 67: a recording `IKafkaClient` fake, Tier 1 route-contract + Tier 2 persistence-round-trip + Tier 3 audit-fired-assertion suites across all 7 services, a real-SIGKILL container-reaping proof, and byte-deterministic OpenAPI generation + CI drift-check), and **165** in `MobileApp.Tests` (158 passing, 7 skipped — the skip is deliberate: `AuthAcceptanceTests` needs a live Identity service reachable at `IDENTITY_BASE_URL`/`localhost:6036` and gracefully `Skip.If`s when nothing's listening, not a bug)
...
- `Booking.Api/`, `Calendar/`, `Customer/`, `Provider/`, `Services/`, `Profession/`, `Identity/` — seven independent ASP.NET Minimal API microservices. **Booking split into 4 projects by F-019** (Clean Architecture pilot — the other 6 are unchanged, one-project-per-service): `Booking.Api/` (thin — endpoint/DI wiring only, was `Booking/`), `Booking.Core/` (MediatR command/query handlers), `Booking.Domain/` (commands/queries/DTOs, `DataResponse<T>`), `Booking.Infrastructure/` (deliberately empty — YAGNI, nothing to put there yet)
...
- **Test (backend, 516 tests):** `dotnet test agenda-buddy-backend.slnf --collect:"XPlat Code Coverage"` — use the solution filter, not the full solution
...
- **Format:** `dotnet format agenda-buddy-backend.slnf` — `.editorconfig` (F-018-T03) now encodes the project's actual conventions (4-space indent, Allman braces, file-scoped namespaces, `var` everywhere, no `this.`, `dotnet_naming_rule.*` for CONSTITUTION §2's naming table at `suggestion` severity so the retroactive reformat couldn't silently rename anything). `build-and-test` runs `dotnet format --verify-no-changes` as a CI gate
- **Regenerate the OpenAPI specs:** `./scripts/generate-openapi.sh [Service…]` → `docs/api/openapi/`. Runs each service **standalone as Development** against a throwaway Mongo container, because Swashbuckle is registered only in Development and the AppHost's services do not run as Development. **The committed `docs/api/openapi/*.json` files are now sourced from a different, byte-deterministic mechanism** (F-018-T16 — `AgendaBuddy.IntegrationTests/OpenApi/OpenApiSpecGenerator.cs`, resolves `ISwaggerProvider` from DI directly, no HTTP call, no container) and are diffed against a live regeneration by `OpenApiSpecDriftTest` (F-018-T17) inside the `integration` CI job. This script remains the manual, on-demand path for a developer without CI access; the two mechanisms produce semantically identical output (confirmed by structural diff), differing only in JSON indentation
...
**Booking is the pilot for a 4-project Clean Architecture layering (F-019), the other 6 services are unchanged.** `Booking.Api` (endpoints/DI only) dispatches via `IMediator` to handlers in `Booking.Core`, typed against `Booking.Domain`'s commands/queries/DTOs; handlers return `FluentResults.Result<T>`, which `Booking.Api` maps to an in-repo `DataResponse<T>` envelope (`Success`/`Data`/`Errors`) at the wire boundary — not a string-sniffed `"exception"`-prefix convention. `Booking.Infrastructure` exists but is deliberately empty (YAGNI — nothing needed it yet). Validation is mid-migration from `MiniValidator` to Validot's `Specification<T>` DSL: 3 of Booking's 10 routes use Validot today (Book, and the 2 note-content routes), 2 still use `MiniValidator` (Update, Cancel — tracked as `agenda-buddy-02e`), the rest use inline checks or have no body. New handlers still audit via `eventStore.SaveAsync` per CONSTITUTION §3; `EventStoreWriteGuardTest`'s `ScanRoots` now covers `Booking.Core` as well as `EventAndCommands`.
...
- `Booking.Api/Program.cs` — representative Minimal API entry point showing the full wiring pattern (F-019 split it from the other 6 services' single-project shape into `Booking.Api`/`Booking.Core`/`Booking.Domain`/`Booking.Infrastructure` — see Architecture above)
...
- `AgendaBuddy.IntegrationTests/` — the only integration suite. `Harness/ServiceHostFixture.cs` hosts a real service over HTTP against a MongoDB Testcontainer (container per test class, database per test); `Harness/MongoEndpointGuard.cs` **fails the suite closed** if the resolved endpoint is not this session's own container. Add a per-service anchor alias to `GlobalUsings.cs` to host a new service — never `WebApplicationFactory<Program>`, which is ambiguous across all seven assemblies (see `Harness/EntryPoints.cs`). F-018 (Waves 1a–2a) filled out the planned tier folders: `Support/` (`KafkaClientFake` — Kafka is never containerised, ADR-017), `Contract/` (Tier 1, one route-contract test per service, status codes only — deliberately not envelopes, since F-019 changes those by design), `Persistence/` (Tier 2, write→read round-trips against the real container, `ConfiguredCollection.Of<T>` reads collection names from config), `Audit/` (Tier 3, audit-fired assertions read via raw `MongoDB.Driver`, never `IEventStore`, plus `EventStoreWriteGuardTest` — a convention-based permanent guard scanning every command/query handler file for the `eventStore.SaveAsync(` call), and `OpenApi/` (`OpenApiSpecGenerator`/`OpenApiSpecCatalog` — resolves `ISwaggerProvider` from DI with no HTTP call and no container, byte-deterministic output committed to `docs/api/openapi/*.json` per ADR-048, plus `OpenApiSpecDriftTest` diffing a live regeneration against that committed baseline)
...
- `docs/api/openapi/` + `scripts/generate-openapi.sh` — generated OpenAPI specs for all 7 services, plus a route index. A **build artifact**, regenerable on demand; do not hand-edit. As of F-018-T16, the `<Service>.json` files are committed (ADR-048 — F-016 shipping cleared ADR-020's earlier "do not commit" deferral) and CI-drift-checked (F-018-T17); `index.md` is still owned by `scripts/generate-openapi.sh` alone and goes stale until that script is rerun by hand
...
- `.github/workflows/dotnet.yml` — CI pipeline: restore → build → test → coverage upload, plus AppHost build and startup guards. As of F-017, also: **`security-scan`** (dependency audit + `gitleaks` secret scan, `if: always()` — runs on every PR unconditionally, no path filter) and **`docker-build-and-scan`** (7-service matrix, `dotnet publish -t:PublishContainer` + Trivy scan, `timeout-minutes: 10`, gated on a `docker` path filter — verified consumed, unlike the still-dead `library` filter output). As of F-018: `build-and-test` also runs `dotnet format --verify-no-changes` (T03); `integration` also runs `scripts/verify-container-reaping.sh` (T15, proves Testcontainers' Ryuk reaper actually cleans up after a mid-flight SIGKILL — not assumed) and picks up `OpenApiSpecDriftTest` (T17) for free since it runs the whole `AgendaBuddy.IntegrationTests` project with no `--filter`. Both new `integration`-job checks and the format-check step are proven red→green locally only as of this session — live-CI confirmation is still owed on a real PR
...
- `scripts/verify-container-reaping.sh` (F-018-T15) — same self-test shape, one more layer down: launches a real `ServiceHostFixture`-backed test in the background, SIGKILLs it the instant its Mongo Testcontainer reports ready (before `DisposeAsync` can run cleanly), then polls `docker inspect` for the exact captured container IDs and fails loudly naming any survivor. Proves Testcontainers' Ryuk reaper actually reaps on this machine — observed (~10s, twice) rather than assumed from documentation, the same "reasoned, not observed" trap F-013 fell into with a different threat. Wired into the `integration` CI job

...
/// illegal transition throws <see cref="InvalidOperationException"/>, which the route maps to <b>409</b>
...
public class ChangeAppointmentStatusCommandHandler(
    ...
    IEventStore eventStore) : IRequestHandler<ChangeAppointmentStatusCommand, string>
{
    public async Task<string> Handle(ChangeAppointmentStatusCommand request, CancellationToken cancellationToken)
    {
        ...
        // Throws InvalidOperationException on an illegal transition — deliberately NOT caught here. The route
        // maps it to 409, and catching it to return a bool would lose the distinction between "illegal
        // transition" (409) and "no such appointment" (404).

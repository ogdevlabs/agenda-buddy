namespace Booking.Core.Queries;
    ...
    : IRequestHandler<GetAppointmentPaymentQuery, Result<PaymentEntity>>
{
    public async Task<Result<PaymentEntity>> Handle(GetAppointmentPaymentQuery request, CancellationToken cancellationToken)
    {
        ...
        if (payment is null)
        {
            ...
            // 404-shaped in Booking.Api. Safe here: by the time this query is reached, the caller has
            // already proven they are a participant in the appointment (Booking.Api's ownership check).

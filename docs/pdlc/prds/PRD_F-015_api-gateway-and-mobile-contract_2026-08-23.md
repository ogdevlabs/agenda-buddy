...
| Traceability | Strong | All 15 ACs (13 + 2 threat-derived `[security]`) map to exactly one task each — AC1→T08, AC2→T07, AC3/AC4→T05, AC5→T04, AC6→T12, AC7→T07, AC8→T08, AC9/AC10→T09, AC11→T10, AC12→T06, AC13→T11, AC14(T-302)→T03, AC15(T-303)→T04. **Named nuance, not a gap:** T01 (scaffold), T02 (spike), T13 (OpenAPI regen), and T14 (closing verification) own no numbered AC directly — each traces to prose in Requirements/ARCHITECTURE.md instead (infrastructure and closing tasks, not orphans; T14 explicitly attests the other tasks' ACs rather than owning new ones). |

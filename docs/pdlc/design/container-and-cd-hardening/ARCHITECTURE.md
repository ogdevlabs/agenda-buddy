...
- **Runs unconditionally on every pull request (`if: always()`), gated on no path filter at all.** **Correction, found at the Party Review gate (2026-08-26), superseding the original design below:** gating this job on the `api` filter — the same trigger `build-and-test` uses — silently skipped it on a docs/scripts/Gateway/MobileApp-only PR. `ISSUE-002`'s original credential leak lived partly under `docs/pdlc/context/`, exactly the path class that gate excluded, so the one job whose entire purpose is catching a leak like that could itself miss it. A secret scanner and a dependency audit are cheap enough to run on every PR regardless of what changed; "docs-only changes run no build jobs" is the right policy for `build-and-test`, not for a security gate. ~~Gated on the existing `api` filter output — the same trigger `build-and-test` already uses, so it fires on exactly the same set of changes.~~ (original, superseded design)
- Step 1: `dotnet list package --vulnerable --include-transitive` against the full `agenda-buddy.sln` (not `agenda-buddy-backend.slnf` — the slnf excludes `AgendaBuddy.IntegrationTests` per ADR-031, and that project is exactly where the one accepted finding lives, so scanning the slnf would give the gate no real coverage). Fails the job on any new HIGH/CRITICAL finding.
- **Correction, found at Construction (F-017-T03), superseding the original assumption below:** the existing `ADR-030` `NU1903` `<NoWarn>` suppression does **not** make `dotnet list package --vulnerable` skip the accepted `SSH.NET` finding — verified live, the finding still appears in the report with the `NoWarn` in place, because `NoWarn` only suppresses the *build-time warning*, not the audit command's *vulnerability report*. The CI step therefore filters the known finding explicitly by its exact advisory ID (`GHSA-q939-rpr3-3284`), not by relying on the MSBuild suppression — confirmed a different/new HIGH finding still fails the gate. ~~The existing `ADR-030` `NU1903` project-scoped suppression... is carried forward unchanged — the audit already respects it, since it's an MSBuild-level suppression, not a CI-script filter.~~ (original, incorrect assumption, struck through rather than deleted)

...
| Suite | Command | Before (F-018 baseline) | After |
|---|---|---|---|
| Backend unit | `dotnet test agenda-buddy-backend.slnf` | 484 | **512** (+28) |
| Integration | `dotnet test AgendaBuddy.IntegrationTests/…csproj` | 301 | **310** (+9) |
| Mobile | `…/MobileApp.Tests.csproj /p:MobileWorkloads=false` | 165 | **165**, untouched (158 pass, 7 deliberately skipped) |
| **Total** | three commands | 950 | **987** |

0 failing. `dotnet format agenda-buddy-backend.slnf --verify-no-changes` clean. Integration suite duration
~4 minutes against the 600s CI budget (ADR-017's tripwire this feature was explicitly flagged as testing —
comfortable margin, not close to the line).

**Backend net +28, not the sum of what each task added**, because F-019-T10 deleted 1 stub-adjacent handler
pair and F-019-T04 deleted `EventsHelperTest.cs` (3 tests) as a direct consequence of `RequestCollection`'s
removal — both disclosed at the task that made them, not surprises here.
...
| 6 | Validot validates every one of Booking's 10 request DTOs; zero `MiniValidator` in `Booking.Api` | `grep -rn "MiniValidator" Booking.Api` finds 2 real calls (`Program.cs:190,215`, Update/Cancel) | ❌ **partially met — disclosed, see §3.5, filed `agenda-buddy-02e`** |
...
| `agenda-buddy-02e` | Update/Cancel routes never migrated `MiniValidator` → Validot | Never assigned to any F-019 task; Requirement 6 partially met |
| `agenda-buddy-2hd` | `PUT /appointments/` response body echoes the client's forged `AppointmentStatus` | Pre-existing, unchanged by this refactor; found while closing AC8's evidence gap, not this task's job to fix |

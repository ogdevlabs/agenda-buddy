...
(`<Service>` = Calendar, Customer, Provider, Services, Profession):

```
<Service>.Api/            (was <Service> — thin: endpoint definitions, DI wiring, no logic)
<Service>.Core/           (new — MediatR command/query handlers, moved from EventAndCommands/Commands/<Service>)
<Service>.Domain/         (new — commands, queries, request DTOs, its own DataResponse<T>)
<Service>.Infrastructure/ (new — thin/possibly empty, same as Booking.Infrastructure — see §6)
```

`<Service>.Tests` stays one project per service (not split per new project), same rationale as Booking's.
Internal namespaces inside `<Service>.Api` stay `<Service>.*` (e.g. `Calendar.Configuration`,
`Calendar.Requests`), **not** renamed to `<Service>.Api.*` — this repeats Booking's own precedent
(`Booking.Configuration` stayed as-is after the project rename, per F-019's `verification.md` §4), applied
uniformly across all 5 rather than re-litigated per service.
...
- Does not touch Booking (already done, F-019) or Identity (excluded, Discover 2026-08-27 — see the PRD's Out of Scope).
- Does not re-wire `MobileApp` to any new envelope shape.

...
| 2026-08-27 | `v0.8.0` | Pulse | 008 (pending) | **Verified** against a live 8-process AppHost run on merged `main`. Gateway `/health`=`Healthy`; anonymous `POST api/v1/booking/appointments` through the Gateway = 401, confirming the `Booking`→`Booking.Api` rename kept the pinned contract intact. Full authenticated round trip through the Gateway: register → login → create Provider → book appointment → update appointment. The new `DataResponse<T>` envelope confirmed live on a real 201 (`{"data":{...},"errors":[],"success":true}`). **`agenda-buddy-2hd`'s fix confirmed under real traffic**: submitted `appointmentStatus: 2` (Completed) on the update, got back `appointmentStatus: 0` (Requested) — the actual persisted value, not the forged one. AppHost stopped cleanly this time (no orphan-process gotcha). |
| 2026-08-27 | `v0.9.0` | Pulse | 009 | **Verified** against a live 8-process AppHost run — all processes now show as `AgendaBud*` in `ps`/`lsof` output (a direct, amusing side-effect of this feature's own rename). Gateway `/health`=`Healthy`; anonymous `POST api/v1/booking/appointments` and both `GET api/v1/calendar/*` routes through the Gateway = 401, confirming both the already-renamed Booking and the newly-migrated Calendar keep their pinned contracts. Real register → create-provider round trip through `AgendaBuddy.Provider.Api`: 201, live `DataResponse<T>` envelope. **Threat T-204's fix confirmed under real traffic**: register → create-customer round trip through `AgendaBuddy.Customer.Api` succeeded (201, real `kafkaTopic` value returned) — proof `AddCustomerCommandHandler`'s `IKafkaClient` retyping resolves cleanly under live MediatR dispatch, not just in a unit test. One false alarm during verification (a "duplicate email" 400 on the first provider-creation attempt) traced to a timestamp collision with a prior run's persistent-volume data, not a real defect — confirmed by a second attempt with a distinct email succeeding cleanly. AppHost stopped cleanly; known orphan-process gotcha recurred (cleaned up by explicit `pkill` on the `AgendaBuddy.*.csproj` pattern). |
...
| 2026-08-27 | `v0.8.0` (F-019): eighth consecutive cloud deploy skip, seventh under the ADR-035 deferral. F-022–F-026 still remain, so the deferral's exit condition is still unmet. **Full live AppHost smoke test run this time** (self-decided, not the F-018 one-line-change precedent) — this feature rewrote Booking's entire dispatch mechanism and response envelope, a production surface too large to trust to CI alone. Real register → login → create-provider → book-appointment → update-appointment round trip through the Gateway on live infrastructure (Mongo + Kafka containers, all 8 processes). Confirmed live: the new `DataResponse<T>` envelope (`{"data":...,"errors":[],"success":true}`) on a real 201 Created; the Party Review fix for `agenda-buddy-2hd` holds under real traffic — a `PUT` submitting a forged `appointmentStatus: 2` (Completed) came back `appointmentStatus: 0` (Requested) in the response body, matching the actual persisted state, not the client's forged value. AppHost stopped cleanly after. Merge CI (`gh run watch` on the `main` push, since `gh pr create` is blocked under this identity — see STATE.md Guardrail Log) also went fully green: all 15 jobs, 0 new findings, unlike F-017's PR CI run which found 4 real defects — none found here | Pulse |
| 2026-08-27 | `v0.9.0` (F-020): ninth consecutive cloud deploy skip, eighth under the ADR-035 deferral. F-022–F-026 still remain. **Full live AppHost smoke test run again** — the largest production surface of any feature to date (5 more services' entire dispatch mechanism, plus a 47-project rename). Confirmed live: Booking's and Calendar's pinned 401 contracts survived their renames; real create-provider and create-customer round trips through the Gateway to the newly-migrated `AgendaBuddy.Provider.Api`/`AgendaBuddy.Customer.Api`, both returning the correct `DataResponse<T>` envelope; **threat T-204's fix confirmed under real Kafka topic-creation traffic**, not just a unit test. The API refactor program (F-018→F-019→F-020) is now complete — this is the last release in that program's own deploy-skip streak; whichever feature ships next starts a fresh count against ADR-035's exit condition | Pulse |

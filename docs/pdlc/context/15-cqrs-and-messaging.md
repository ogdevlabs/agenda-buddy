...
⚠️ **`kafkaClient as KafkaClient`** (`Booking/Requests/RequestCollection.cs:11,23,33`, `Customer/Requests/RequestCollection.cs:11`) casts the injected `IKafkaClient` down to the concrete class, because those handlers declare `KafkaClient?` rather than `IKafkaClient`. If a different `IKafkaClient` implementation were ever registered (a test double, a null-object), the cast yields `null` silently, and the null-forgiving `!` at each call site hides it from the compiler. Both are latent, not live — nothing currently substitutes Booking's or Customer's `IKafkaClient` registration, so the defect has never fired.
>
> **F-018-T10 fixed the Provider instance of this defect** (the one F-018-T10's `KafkaClientFake` actually exercises): `AddProviderCommandHandler`'s constructor parameter is now `IKafkaClient` instead of `KafkaClient`, and `Provider/Requests/RequestCollection.cs:12` passes `kafkaClient` straight through with no cast. It was found live, not by inspection: `KafkaClientFake`, substituted via `ConfigureTestServices`, made `(kafkaClient as KafkaClient)!` evaluate to `null` and `AddProviderCommandHandler` `NullReferenceException` on the real `POST /api/v1/providers` path, a 500 where AC1 expected a recorded topic-creation call. The identical pattern in Booking and Customer was deliberately left alone — same defect, still real, but dormant — to keep the fix scoped to what F-018-T10 actually exercised. Follow-up tracked separately.
